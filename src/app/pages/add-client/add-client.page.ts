...
export class AddClientPage implements OnInit {
  ...
  opt3: any;
  opt4: any;
  userImage_Send: string = null;
  aadhaar_front_Send: string = null;
  aadhaar_back_Send: string = null;
  pan_Send: string = null;
  ca_Send: string = null;
  salary_slip_Send: string = null;
  passbook: string = null;
  passbook_Send: string = null;
    ...
    this.addClientForm = this.formBuilder.group(
      {
        ...
        passbook: new FormControl('', Validators.compose([])),
  ...
  getImg(options: CameraOptions, flag: any) {
    this.camera.getPicture(options).then((imageData) => {
      if (flag == 1) {
        ...
        this.userImage_Send = imageData;
      } else if (flag == 2) {
        this.aadhaar_front = 'data:image/jpeg;base64,' + imageData;
        this.aadhaar_front_Send = imageData;
      } else if (flag == 3) {
        this.aadhaar_back = 'data:image/jpeg;base64,' + imageData;
        this.aadhaar_back_Send = imageData;
      } else if (flag == 4) {
        this.pan = 'data:image/jpeg;base64,' + imageData;
        this.pan_Send = imageData;
      } else if (flag == 5) {
        this.ca = 'data:image/jpeg;base64,' + imageData;
        this.ca_Send = imageData;
      } else if (flag == 6) {
        this.salary_slip = 'data:image/jpeg;base64,' + imageData;
        this.salary_slip_Send = imageData;
      } else if (flag == 7) {
        this.passbook = 'data:image/jpeg;base64,' + imageData;
        this.passbook_Send = imageData;
      } else {
  ...
  async verify() {
    try {
      ...
      if (this.addClientForm.valid) {
        ...
        //   'image': this.userImage_Send,
        //   'aadhar_back_image': this.aadhaar_back_Send,
        //   'aadhar_front_image': this.aadhaar_front_Send,
        //   'ca_report': this.ca_Send,
        //   'pan_card_image': this.pan_Send,
        //   'salary_slip': this.salary_slip_Send,
        //   'passbook': this.passbook_Send,
  ...
  OTPVerify() {
    ...
    if (otpPin.length == 4) {
      ...
      this.commonService.hideLoader();
      this.commonService.showAlert(AlertTypeEnum.Error, 'Invalid OTP.');
    }
  }
